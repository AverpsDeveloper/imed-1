...
function ListTagsPage() {
  ...
  const deleteHandler = (id) => {
    setLoading(true);
    axios.delete('/api/tags', { data: { id } }).then((respocnse) => {
      setLoading(false);
  ...
  const editHandler = (editable) => {
    if (editable) router.push("/dashboard/inventory/tags/add?id=" + editable)
  };
  ...
  useEffect(() => {
    ...
    axios.get('/api/tags')
      .then((response) => {
        setTags(response.data.result.data);
        setLoading(false);
      ...
      <div className="bg-white w-full h-full p-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold">Tags List</h1>
            <Link href="/dashboard/inventory/tags/add"
              ...
              <tbody>
                {tags.map((tag, index) => (
                  ...
                  <tr key={tag._id}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p
                        className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${tag.isActive ? "text-success bg-success" : "text-danger bg-danger"
                          }`}
                      >
                        {tag.isActive ? "Active" : "Deactive"}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button
                          onClick={() => editHandler(tag._id)}
