...
function BatchPageListing() {
  ...
  const { page, limit, search, date, searchHandler, setSearchParmas, product } = usePaginate();
  const [itemOptions, setItemOptions] = useState([]);


  useEffect(() => {
    api.get('item-batch', { params: { page, limit, search, date, product } })
      .then((response) => {
        setBatch(response.data.result.data);
      })
  }, [page, limit, search, date, product]);
        ...
        <div className="inline">
          ...
          <AsyncSelect
            ...
            onChange={(selectedOption) => {
              setSearchParmas("product", selectedOption?.id ?? "");
            }}
            value={itemOptions.find(d => d.id == product)}
          />
